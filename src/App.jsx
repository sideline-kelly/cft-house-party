...
const DEFAULT_TEAMS = [
  { id: "t01", heat: "HEAT 1", lane: "A", teamName: "Rich & Melissa",   athlete1: "Rich",    athlete2: "Melissa",  athlete3: "", athlete4: "", division: "" },
  { id: "t02", heat: "HEAT 1", lane: "B", teamName: "Scott & Ilsia",    athlete1: "Scott",   athlete2: "Ilsia",    athlete3: "", athlete4: "", division: "" },
  { id: "t03", heat: "HEAT 1", lane: "D", teamName: "Ben & Rachel",     athlete1: "Ben",     athlete2: "Rachel",   athlete3: "", athlete4: "", division: "" },
  { id: "t04", heat: "HEAT 1", lane: "E", teamName: "Brenda & Colin",   athlete1: "Brenda",  athlete2: "Colin",    athlete3: "", athlete4: "", division: "" },
  { id: "t05", heat: "HEAT 1", lane: "F", teamName: "Cam & Britt",      athlete1: "Cam",     athlete2: "Britt",    athlete3: "", athlete4: "", division: "" },
  { id: "t06", heat: "HEAT 2", lane: "A", teamName: "Haley & Daniel",   athlete1: "Haley",   athlete2: "Daniel",   athlete3: "", athlete4: "", division: "" },
  { id: "t07", heat: "HEAT 2", lane: "B", teamName: "Sara & Carlos",    athlete1: "Sara",    athlete2: "Carlos",   athlete3: "", athlete4: "", division: "" },
  { id: "t08", heat: "HEAT 2", lane: "D", teamName: "Matt & Alyssa",    athlete1: "Matt",    athlete2: "Alyssa",   athlete3: "", athlete4: "", division: "" },
  { id: "t09", heat: "HEAT 2", lane: "E", teamName: "Staci & Stephen",  athlete1: "Staci",   athlete2: "Stephen",  athlete3: "", athlete4: "", division: "" },
  { id: "t10", heat: "HEAT 3", lane: "A", teamName: "Oscar & Aly",      athlete1: "Oscar",   athlete2: "Aly",      athlete3: "", athlete4: "", division: "" },
  { id: "t11", heat: "HEAT 3", lane: "B", teamName: "Michael & Amy",    athlete1: "Michael", athlete2: "Amy",      athlete3: "", athlete4: "", division: "" },
  { id: "t12", heat: "HEAT 3", lane: "D", teamName: "Matt & Charity",   athlete1: "Matt",    athlete2: "Charity",  athlete3: "", athlete4: "", division: "" },
  { id: "t13", heat: "HEAT 3", lane: "E", teamName: "Ronnie & Kayla",   athlete1: "Ronnie",  athlete2: "Kayla",    athlete3: "", athlete4: "", division: "" },
  { id: "t14", heat: "HEAT 4", lane: "A", teamName: "Dan & Eugenia",    athlete1: "Dan",     athlete2: "Eugenia",  athlete3: "", athlete4: "", division: "" },
  { id: "t15", heat: "HEAT 4", lane: "B", teamName: "Kat & Gia",        athlete1: "Kat",     athlete2: "Gia",      athlete3: "", athlete4: "", division: "" },
  { id: "t16", heat: "HEAT 4", lane: "D", teamName: "Robbie & Tiffany", athlete1: "Robbie",  athlete2: "Tiffany",  athlete3: "", athlete4: "", division: "" },
  { id: "t17", heat: "HEAT 4", lane: "E", teamName: "Sabrey & Everett", athlete1: "Sabrey",  athlete2: "Everett",  athlete3: "", athlete4: "", division: "" },
];
...
function DayBadge({ label }) {
  return (
    <div style={{ display: "inline-block", background: "#c0392b", ...T.badge, color: "#fff", padding: "3px 10px", borderRadius: 3, marginBottom: "0.6rem" }}>
      {label}
    </div>
  );
}
...
function HeatBadge({ heat }) {
  return (
    <span style={{ ...T.badge, background: HEAT_COLORS[heat]?.bg || "#444", color: HEAT_COLORS[heat]?.text || "#fff", padding: "3px 10px", borderRadius: 3, flexShrink: 0 }}>
      {heat}
    </span>
  );
}
...
function DivisionBadge({ division, show }) {
  ...
  const colors = { RX: "#3b82f6", Intermediate: "#06b6d4", Scaled: "#8b5cf6", Masters: "#f59e0b", Mixed: "#ec4899" };
...
function SectionCard({ children, style = {} }) {
  return (
    <div style={{ background: "#141414", border: "1px solid #222", borderRadius: 10, overflow: "hidden", marginBottom: "1.25rem", ...style }}>
      {children}
    </div>
  );
}
...
function RemoveBtn({ onClick }) {
  return (
    <button onClick={onClick} style={{ background: "none", border: "1px solid #3a1a1a", borderRadius: 4, color: "#c0392b", cursor: "pointer", fontFamily: inter, fontSize: 11, fontWeight: 700, padding: "0.25rem 0.5rem" }}>
      ✕
    </button>
  );
}
...
function WodEditor({ wod, onChange, onRemove }) {
  ...
  return (
    <SectionCard>
      <SectionHeader title={wod.name || "Untitled WOD"} action={<RemoveBtn onClick={onRemove} />} />
...
// NOTE: showDivisions is passed as a prop so this component stays self-contained
function TeamProfileCard({ team, onUpdate, onRemove, showDivisions }) {
    ...
    <div style={{ background: "#161616", border: "1px solid #242424", borderRadius: 8, overflow: "hidden" }}>
      <button onClick={() => setExpanded(e => !e)}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexShrink: 0 }}>
          {/* Always show division badge in admin panel regardless of global toggle */}
          <DivisionBadge division={team.division} show={true} />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              {[1, 2, 3, 4].map(n => (
                <div key={n} style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: "0.5rem", alignItems: "center" }}>
                  ...
                  <Field value={team[`athlete${n}`] || ""} onChange={v => onUpdate({ [`athlete${n}`]: v })} placeholder={`Athlete ${n} name`} />
                </div>
              ))}
            </div>
          </div>
...
export default function CFTCompApp() {
    ...
    async function init() {
      try {
        ...
        if (!s || !Object.keys(s).length) {
          setScores(initScores(t?.length ? t : DEFAULT_TEAMS, w?.length ? w : DEFAULT_WODS));
        }
      } catch (e) {
  ...
  const showDivisions = settings.showDivisions === "true";
  ...
  function addTeam() {
    setTeams(prev => [...prev, { id: uid(), heat: "HEAT 1", lane: "", teamName: "", athlete1: "", athlete2: "", athlete3: "", athlete4: "", division: "" }]);
    setSyncStatus("unsaved");
  }
  ...
  function addWod() {
    setWods(prev => [...prev, { id: uid(), name: "New WOD", cap: "", description: [], notes: [], heats: [{ heat: "HEAT 1", time: "" }] }]);
    setSyncStatus("unsaved");
  }
  ...
  async function handleSync() {
    ...
    try {
      await Promise.all([saveTeams(teams), saveScores(scores), saveSettings(settings), saveWods(wods)]);
        ...
        <div style={{ display: "flex", gap: "0.25rem", padding: "0.75rem 0" }}>
          {["schedule", "leaderboard", "admin"].map(t => (
            <button key={t} onClick={() => setTab(t)}
              style={{ ...T.tabText, padding: "0.45rem 1.1rem", borderRadius: 4, border: "none", cursor: "pointer", background: tab === t ? "#c0392b" : "transparent", color: tab === t ? "#fff" : "#777", transition: "all 0.15s" }}>
              {t === "admin" ? "🔒 Admin" : t}
            </button>
          ))}
        </div>
        ...
        <div style={{ padding: "1.75rem 1.5rem" }}>
                ...
                {(wod.description?.length > 0 || wod.notes?.length > 0) && (
                  <div style={{ padding: "0.75rem 1.25rem", borderBottom: "1px solid #1c1c1c", background: "#131313" }}>
                    {(wod.description || []).map((line, i) => (
                      <div key={i} style={{ ...T.body, fontSize: 13, color: "#ccc" }}>{line || <>&nbsp;</>}</div>
                    ))}
                    {wod.notes?.length > 0 && (
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem", padding: "0.5rem 1.25rem 0.65rem", paddingLeft: "calc(1.25rem + 74px)" }}>
                      {h.teams.map(team => (
                        <span key={team.id} style={{ display: "inline-flex", alignItems: "center", gap: "0.35rem", ...T.teamMuted, background: "#1e1e1e", border: "1px solid #282828", borderRadius: 4, padding: "3px 10px" }}>
                          {team.teamName}
                          <DivisionBadge division={team.division} show={showDivisions} />
                        </span>
                      ))}
                    </div>
                  </div>
          ...
          <div style={{ border: "1px solid #222", borderRadius: 8, overflow: "hidden" }}>
            <button onClick={() => setAccordionOpen(o => !o)}
              style={{ width: "100%", background: "#161616", border: "none", padding: "0.8rem 1.25rem", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", textAlign: "left" }}>
              <div>
                      ...
                      <div style={{ padding: "0.5rem 0.75rem", display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                        {teams.filter(t => t.heat === heat).map(t => (
                          <div key={t.id} style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                            ...
                            <DivisionBadge division={t.division} show={showDivisions} />
                          </div>
                        ))}
                      </div>
                    </div>
                      ...
                      <tr key={row.id} style={{ background: rowBg, borderBottom: "1px solid #1a1a1a" }}>
                        ...
                        <td style={{ padding: "0.65rem 0.75rem" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                            ...
                            <DivisionBadge division={row.division} show={showDivisions} />
                          </div>
        ...
        !adminUnlocked ? (
          <div style={{ padding: "1.75rem 1.5rem" }}>
            <div style={{ maxWidth: 340, margin: "3rem auto", background: "#161616", border: "1px solid #222", borderRadius: 10, padding: "2.5rem 2rem", textAlign: "center" }}>
              ...
              <button onClick={handleLogin} style={{ width: "100%", padding: "0.65rem", background: "#c0392b", border: "none", borderRadius: 4, color: "#fff", fontFamily: inter, fontSize: 15, fontWeight: 700, cursor: "pointer", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                Unlock
              </button>
            </div>
          </div>
          ...
          <div>
              ...
              <div style={{ padding: "1.75rem 1.5rem" }}>
                ...
                <SectionCard>
                  ...
                  <div style={{ padding: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                    ...
                    <Field label="Check-In Info" value={settings.checkinInfo} onChange={v => updateSettings({ checkinInfo: v })} placeholder="Check-in 8:00–8:20am · Athlete Brief 8:30am" />
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#0d0d0d", border: "1px solid #2e2e2e", borderRadius: 5, padding: "0.55rem 0.75rem" }}>
                      ...
                      <button
                        onClick={() => updateSettings({ showDivisions: showDivisions ? "false" : "true" })}
                        style={{ width: 44, height: 24, borderRadius: 12, border: "none", cursor: "pointer", flexShrink: 0, background: showDivisions ? "#22c55e" : "#333", position: "relative", transition: "background 0.2s" }}>
                        <span style={{ position: "absolute", top: 3, width: 18, height: 18, borderRadius: "50%", background: "#fff", transition: "left 0.2s", left: showDivisions ? 23 : 3 }} />
                      </button>
                    </div>
                  </div>
                </SectionCard>
            ...
            {adminTab === "wods" && (
              <div style={{ padding: "1.75rem 1.5rem" }}>
                ...
                <div style={{ ...T.body, marginBottom: "1.5rem" }}>Edit workout names, formats, movements, scaling notes, and heat times.</div>

                {wods.map((wod, idx) => (
                  <WodEditor key={wod.id} wod={wod}
            ...
            {adminTab === "teams" && (
              <div style={{ padding: "1.75rem 1.5rem" }}>
                ...
                <div style={{ ...T.body, marginBottom: "1.5rem" }}>Edit team names, athletes (up to 4), division, heat, and lane.</div>
                        ...
                        {teams.map(team => {
                          ...
                          return (
                            <tr key={team.id} style={{ borderBottom: "1px solid #1a1a1a" }}>
                              ...
                              <td style={{ padding: "0.6rem 0.75rem" }}>
                                {/* Always show division in admin scoring table */}
                                <DivisionBadge division={team.division} show={true} />
                              </td>
                              {wods.map(w => (
